...
export const chapters = [start[0], ...compact, ...advanced, ...state, ...integration, ...tools, npmChapters.ko, ...securityPlatformChapters.ko, start[1]];
...
const translated = chapters.map(c=>c.slug==='npm'?npmChapters.en:(securityPlatformChapters.en.find(x=>x.slug===c.slug)??translateChapter(c)));
