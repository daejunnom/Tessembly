...
export const integration = [{
  ...
  sections: [
    ...
    { id: 'tck', title: '자신의 프로그램에 테스트 포트 연결', blocks: [code('cargo build --locked --workspace\n# 자신의 실행 파일과 인수로 교체합니다.\ncargo run --locked -p tessembly-conformance -- \\\n  --report conformance-report.json -- my-host test-port'), p('기본 포트는 tessembly.test-port.v1이며 요청에 id, protocol, profile, op를 넣습니다. capabilities, compile, enumerate_D, evaluate_U_witness, encode/decode, hold_step, check_adapter, resolve_config를 다룹니다. stdout은 JSON Lines 응답 전용이고 로그는 stderr를 사용합니다.'), code('{"id":1,"protocol":"tessembly.test-port.v1","profile":"tessembly.rfc2.precedence.v1","op":"compile","text":"P4:D(I<TS)"}', 'json'), p('고급 문서는 별도 tessembly.document-test-port.v1과 참조 CLI doc-port를 사용합니다. validate, roundtrip, project, decode, hold, resolve를 지원합니다. project는 표준 AST를 투영할 뿐 환경을 실행하지 않습니다. 반환된 문서 환경을 잃지 않고 함께 전달하세요.'), note('테스트 러너는 신뢰한 개발용 실행 파일을 구동하는 도구이며 샌드박스가 아닙니다. 통과 보고서는 해당 버전과 시험 범위에 대한 증거입니다. 외부 앱이나 데이터셋을 자동 인증하는 기능이 아닙니다.')] },
