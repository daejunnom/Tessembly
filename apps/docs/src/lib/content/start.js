...
export const start = [{
  ...
  sections: [
    { id: 'first', title: '처음 쓰는 한 줄', blocks: [code('P4:D(I<TS)', 'tessembly'), p('표준 일곱 종류에서 중복 없이 네 개를 고르는 패턴입니다. T와 S가 I보다 먼저 등장해야 하고, T와 S 사이의 순서는 정하지 않습니다. I가 범위에 없어도 두 선행 종류가 있으면 조건을 만족합니다.'), note('RFC2에서 A>B는 A 선행, A<B는 B 선행입니다. 위치 숫자의 대소가 아닙니다. 과거 RFC1 파일을 같은 의미로 자동 해석하지 않습니다.')] },
  ...
  sections: [
    ...
    { id: 'versions', title: '호환성은 버전으로', blocks: [p('RFC2 의미 프로필, 고급 문서 스키마, 바이너리 wire, 테스트 포트 버전은 서로 별개입니다. RFC1은 자동 변환하지 않고 명시적으로 거부합니다. 커스텀 실행이나 참조 해석을 지원하지 않는 소비자는 조건을 버리지 말고 UNSUPPORTED 또는 UNSUPPORTED_STATE를 반환해야 합니다.')] }
  ]
}];
