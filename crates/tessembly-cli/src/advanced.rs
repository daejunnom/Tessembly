...
fn text(path: &str) -> Result<String> {
    String::from_utf8(crate::read_file(path, tessembly_core::MAX_INPUT)?)
        .map_err(|_| Error::new("INVALID_UTF8"))
}
fn report(d: &Document) -> Result<Value> {
    ...
    Ok(
        json!({"status":"OK","complete":true,"schema":DOCUMENT_SCHEMA,"profile":PROFILE,
...
fn handle(req: &Value) -> Result<Value> {
    if str_field(req, "protocol")? != PROTOCOL || str_field(req, "profile")? != PROFILE {
    ...
    let extra: &[&str] = match op {
        "capabilities" => &[],
        "validate" | "roundtrip" | "project" => &["text"],
        "decode" => &["hex"],
        "hold" => &["text", "action"],
        "resolve" => &["text", "host"],
    ...
    let map = req
        ...
        .ok_or_else(|| Error::new("INVALID_REQUEST"))?;
    for key in map.keys() {
        if !["id", "protocol", "profile", "op"].contains(&key.as_str())
    ...
    if op == "capabilities" {
        return Ok(
            json!({"status":"OK","complete":true,"schema":DOCUMENT_SCHEMA,
    ...
    if op == "decode" {
        ...
        return Ok(
            json!({"status":"OK","complete":true,"hex":crate::port::hex(&wire::encode(&d)?),
    ...
    match op {
        ...
        "roundtrip" => {
            let b = wire::encode(&d)?;
            let back = wire::decode(&b)?;
            let mut r = report(&back)?;
            r["structurally_equal"] = json!(d == back);
            r["hex"] = json!(crate::port::hex(&b));
            Ok(r)
        }
        "project" => Ok(
            json!({"status":"OK","complete":true,"pattern":tessembly_text::format(&d.standard_pattern()?)?,
            "projection_only":true,"environment":d.to_text()?,"execution_checked":false}),
        ),
        "resolve" => {
            let h = parse(str_field(req, "host")?)?;
            report(&d.with_host_config(&h.config)?)
        }
        "hold" => {
            let mut s = config::hold_state(&d.config)?;
            let p = config::hold_policy(&d.config)?;
            let before = s.clone();
            ...
            let held = match s.held {
                tessembly_core::hold::Slot::None => json!("NONE"),
                tessembly_core::hold::Slot::Empty => json!("EMPTY"),
                tessembly_core::hold::Slot::Occupied(t) => {
                    json!({"piece":t.kind.to_string(),"origin":t.origin})
                }
            };
            Ok(
                json!({"status":if pending {"NOT_CHECKED"} else if reason.is_none() {"OK"} else {"REJECTED_ACTION"},"complete":!pending,
...
fn error_status(code: &str) -> &'static str {
    if code == "UNSUPPORTED_STATE" {
        "UNSUPPORTED_STATE"
    } else if code.starts_with("UNSUPPORTED") || code.ends_with("REQUIRES_HOST") {
        "UNSUPPORTED"
    } else if code == "CONFIG_CONFLICT" {
        "CONFIG_CONFLICT"
    } else if matches!(code, "CONFIG_REQUIRED" | "STATE_REQUIRED") {
        "NOT_CHECKED"
    } else if code.ends_with("LIMIT") {
        "INCOMPLETE"
    } else if matches!(code, "INVALID_REQUEST" | "UNKNOWN_FIELD") {
        "INVALID_REQUEST"
    } else {
        "INVALID_DOCUMENT"
    }
...
pub fn response(req: &Value) -> Value {
    ...
    out["id"] = req.get("id").cloned().unwrap_or(Value::Null);
...
fn port() -> Result<()> {
    ...
    loop {
        ...
        if n > 2_200_000 {
            return Err(Error::new("REQUEST_LIMIT"));
        }
        let out = match serde_json::from_slice::<Value>(&line) {
            Ok(req) => response(&req),
            Err(_) => {
                json!({"id":null,"protocol":PROTOCOL,"profile":PROFILE,"status":"INVALID_REQUEST","complete":false})
            }
        };
...
pub fn run(command: &str, args: Vec<String>) -> Result<u8> {
    ...
    let deny = args.iter().any(|s| s == "--deny-unsat");
    let mut exit = 0;
    let mut files = 0;
    for path in &args {
        if path == "--deny-unsat" {
        ...
        files += 1;
        let d = parse(&text(path)?)?;
        if command == "doc-format" {
            print!("{}", d.to_text()?);
        } else {
            let r = report(&d)?;
            if deny && (r["feasibility"]["draw"] == "UNSAT" || r["feasibility"]["use"] == "UNSAT") {
                exit = 1;
            }
            println!("{r}");
        }
