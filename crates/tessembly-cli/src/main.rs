        ...
        let response = match serde_json::from_slice(&line) {
            Ok(req) => {
                ...
                if req["op"] == "capabilities" && response["status"] == "OK" {
                    ...
                    response["not_implemented_scope"] =
                        json!("this compact port only; advanced documents use doc-port");
                    response["document_port"] = json!("tessembly.document-test-port.v1");
                }
                response
            }
            Err(_) => {
                json!({"protocol":PROTOCOL,"profile":PROFILE,"id":null,"status":"INVALID_REQUEST","complete":false,"error":{"code":"INVALID_JSON"}})
            }
        };
...
fn run(mut args: Vec<String>, language: locale::Language) -> Result<u8> {
    ...
    if command == "decode" {
        ...
        eprintln!(
            "{}: {PROFILE}",
            if language == locale::Language::Ko {
                "복원한 프로필"
            } else {
                "Decoded profile"
            }
    ...
    if !matches!(command.as_str(), "lint" | "check" | "format" | "encode") {
        return Err(Error::new("INVALID_ARGUMENTS"));
    }
    let mut deny_unsat = false;
...
fn main() -> ExitCode {
    ...
    let result = locale::arguments(&mut args).and_then(|selected| {
        language = selected;
        run(args, language)
    });
    match result {
        ...
        Err(error) => {
            eprintln!(
                "{} [{}] {}..{}",
                locale::message(error.code, language),
                error.code,
                error.span.start,
                error.span.end
            );
