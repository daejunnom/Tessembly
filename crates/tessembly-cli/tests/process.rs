...
fn actual_binary_port_preserves_request_and_profile() {
    let mut child = Command::new(env!("CARGO_BIN_EXE_tessembly"))
        .arg("test-port")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .spawn()
    ...
    assert_eq!(v["id"], 42);
    assert_eq!(v["count"], 176);
    assert_eq!(v["complete"], true);
}
#[test]
fn help_has_version_direction_and_presence() {
    let out = Command::new(env!("CARGO_BIN_EXE_tessembly"))
        .args(["--lang", "ko", "help"])
        .output()
        .unwrap();
...
fn help_uses_primary_locale_with_english_fallback() {
    for (value, expected) in [
        ...
        let out = Command::new(env!("CARGO_BIN_EXE_tessembly"))
            .arg("help")
            .env("TESSEMBLY_LANG", value)
            .output()
            .unwrap();
        assert!(out.status.success());
        assert!(String::from_utf8(out.stdout).unwrap().contains(expected));
    }
}
#[test]
fn explicit_language_overrides_environment_and_errors_keep_codes() {
    let out = Command::new(env!("CARGO_BIN_EXE_tessembly"))
        .args(["--lang", "en", "help"])
        .env("TESSEMBLY_LANG", "ko")
        .output()
        .unwrap();
    assert!(String::from_utf8(out.stdout)
        .unwrap()
        .contains("English help"));
    for language in ["en", "ko"] {
        let out = Command::new(env!("CARGO_BIN_EXE_tessembly"))
            .args(["--lang", language, "invalid-command"])
            .output()
            .unwrap();
