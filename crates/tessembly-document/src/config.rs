    ...
    if let Some(v) = config.get("registry") {
        for id in v.list()? {
            ...
            if !set.insert(id.into()) {
                return Err(Error::new("DUPLICATE_PIECE_ID"));
            }
        }
    }
    Ok(set)
...
fn enum_value(v: &Value, choices: &[&str]) -> Result<()> {
    if !choices.contains(&v.text()?) {
        return Err(Error::new("INVALID_SETTING"));
    }
    Ok(())
}
fn token_check(v: &Value, registry: &BTreeSet<String>) -> Result<()> {
    let c = v.call()?;
    if c.name != "token" {
        return Err(Error::new("EXPECTED_TOKEN"));
    }
    c.arity(1)?;
    c.keys(&["origin"])?;
    if !registry.contains(c.args[0].text()?) {
        return Err(Error::new("UNREGISTERED_PIECE"));
    }
    c.named
        .get("origin")
        .ok_or_else(|| Error::new("ORIGIN_REQUIRED"))?
        .number()?;
    Ok(())
}
fn observation(c: &Call) -> Result<()> {
    match c.name.as_str() {
        ...
        "view" => {
            c.arity(0)?;
            c.keys(&["active", "next", "hold", "memory", "reveal", "bag"])?;
            for key in ["active", "hold"] {
                if let Some(v) = c.named.get(key) {
                    v.boolean()?;
                }
            }
            if let Some(v) = c.named.get("next") {
                if v.number()? > MAX_DRAWS as u64 {
                    return Err(Error::new("VIEW_LIMIT"));
                }
            }
            if let Some(v) = c.named.get("memory") {
                enum_value(v, &["history", "current"])?;
            }
            if let Some(v) = c.named.get("reveal") {
                enum_value(v, &["supply", "lock", "host"])?;
            }
            if let Some(v) = c.named.get("bag") {
                enum_value(v, &["known", "hidden"])?;
            }
        }
        "external" => external(c)?,
        ...
        match key.as_str() {
            ...
            "start" => {
                ...
                match c.name.as_str() {
                    ...
                    "remainder" => {
                        c.arity(1)?;
                        c.keys(&["epoch"])?;
                        ids(&c.args[0], registry)?;
                        if let Some(n) = c.named.get("epoch") {
                            n.number()?;
                        }
                        if config
                            .get("rule")
                            .and_then(|v| v.call().ok())
                            .is_some_and(|r| r.name == "seven_bag")
                        {
                            ...
                            if unique.len() != ps.len()
                                || ps
                                    .iter()
                                    .any(|p| p.len() != 1 || !b"IOTSZJL".contains(&p.as_bytes()[0]))
                            {
            ...
            "hold" => {
                ...
                match c.name.as_str() {
                    ...
                    "slot" => {
                        c.arity(0)?;
                        c.keys(&["initial", "used", "allowed", "deny"])?;
                        let initial = c
                            .named
                            .get("initial")
                            .ok_or_else(|| Error::new("HOLD_INITIAL_REQUIRED"))?;
                        if initial.text().ok() != Some("empty") {
                            token_check(initial, registry)?;
                        }
                        ...
                        if let Some(v) = c.named.get("deny") {
                            let ps = ids(v, registry)?;
                            let unique: BTreeSet<_> = ps.iter().collect();
                            if ps.len() != unique.len() {
...
pub fn resolve(
    ...
    for (k, v) in document {
        if result.get(k).is_some_and(|old| old != v) {
            return Err(Error::new("CONFIG_CONFLICT"));
        }
        result.insert(k.clone(), v.clone());
    }
    for key in ["rule", "start", "see", "hold"] {
        if !result.contains_key(key) {
            return Err(Error::new("CONFIG_REQUIRED"));
        }
    }
    validate(&result, &registry(&result)?)?;
    Ok(result)
}
...
pub fn view(v: &Value) -> Result<Option<View>> {
    ...
    if c.name != "view" {
        return Err(Error::new("UNSUPPORTED_OBSERVATION_PROFILE"));
    }
    ...
    let text = |key, default: &str| {
        c.named
            .get(key)
            .map(Value::text)
            .transpose()
            .map(|v| v.unwrap_or(default).to_owned())
    };
    Ok(Some(View {
        ...
        hold: boolean("hold", true)?,
        memory: text("memory", "history")?,
        reveal: text("reveal", "supply")?,
        bag: text("bag", "known")?,
    }))
}
fn piece(id: &str) -> Result<Piece> {
    if id.len() != 1 {
        return Err(Error::new("UNSUPPORTED_CUSTOM_PIECE_EXECUTION"));
    }
...
fn token(v: &Value) -> Result<Token> {
    ...
    Ok(Token {
        kind: piece(
            c.args
                .first()
                .ok_or_else(|| Error::new("EXPECTED_TOKEN"))?
                .text()?,
        )?,
        origin: c
            .named
...
pub fn hold_policy(config: &BTreeMap<String, Value>) -> Result<HoldPolicy> {
    ...
    let c = config
        .get("hold")
        .ok_or_else(|| Error::new("CONFIG_REQUIRED"))?
        .call()?;
    if c.name == "none" {
        return HoldPolicy::new(false, []);
    }
    if c.name != "slot" {
        return Err(Error::new("UNSUPPORTED_HOLD_PROFILE"));
    }
    let allowed = c
        .named
        .get("allowed")
    ...
    let deny = c
        .named
        .get("deny")
        .map(|v| ids(v, &registry(config)?))
        .transpose()?
        .unwrap_or_default();
    let overrides = deny
        .iter()
        .map(|s| piece(s).map(|p| (p, false)))
...
pub fn hold_state(config: &BTreeMap<String, Value>) -> Result<HoldState> {
    ...
    let s = HoldState {
        ...
        queue: get("queue")?
            ...
            .map(token)
            .collect::<Result<_>>()?,
        cursor: get("cursor")?.number()? as usize,
        tail: if get("tail")?.text()? == "end" {
            Tail::End
        } else {
            Tail::Pending
        },
