...
fn number(out: &mut Vec<u8>, n: usize) -> Result<()> {
    put(
        out,
        &u32::try_from(n)
            .map_err(|_| Error::new("INTEGER_OVERFLOW"))?
            .to_le_bytes(),
    )
}
fn bytes(out: &mut Vec<u8>, b: &[u8]) -> Result<()> {
    number(out, b.len())?;
    put(out, b)
}
fn string(out: &mut Vec<u8>, s: &str) -> Result<()> {
...
fn value(out: &mut Vec<u8>, v: &Value) -> Result<()> {
    match v {
        ...
        Value::Text(s) | Value::Symbol(s) => {
            put(out, &[if matches!(v, Value::Text(_)) { 3 } else { 4 }])?;
            string(out, s)
        }
        Value::List(xs) => {
            put(out, &[5])?;
        ...
        Value::Pattern(n) => {
            ...
            bytes(
                out,
                &tessembly_codec::encode(&tessembly_codec::Document {
                    root: *n.clone(),
...
pub fn encode(doc: &Document) -> Result<Vec<u8>> {
    ...
    predicates(&mut body, &doc.draw)?;
    predicates(&mut body, &doc.use_order)?;
    calls(&mut body, &doc.references)?;
    calls(&mut body, &doc.decisions)?;
    let mut out = MAGIC.to_vec();
    number(&mut out, 1 + doc.optional_extensions.len())?;
    number(&mut out, 1)?;
    put(&mut out, &[1])?;
    bytes(&mut out, &body)?;
    for e in &doc.optional_extensions {
        put(&mut out, &e.id.to_le_bytes())?;
        put(&mut out, &[0])?;
        bytes(&mut out, &e.bytes)?;
    }
    Ok(out)
}
struct Reader<'a, 'b> {
...
impl<'a> Reader<'a, '_> {
    ...
    fn number(&mut self) -> Result<usize> {
        let a: [u8; 4] = self
            .take(4)?
            .try_into()
            .map_err(|_| Error::new("TRUNCATED_BINARY"))?;
    ...
    fn values(&mut self, depth: usize) -> Result<Vec<Value>> {
        let count = self.count(MAX_NODES)?;
        let mut out = Vec::new();
        for _ in 0..count {
    ...
    fn mapping(&mut self, depth: usize) -> Result<BTreeMap<String, Value>> {
        ...
        for _ in 0..count {
            ...
            if out.insert(key, val).is_some() {
                return Err(Error::new("DUPLICATE_KEY"));
            }
        }
        Ok(out)
    }
    fn value(&mut self, depth: usize) -> Result<Value> {
        ...
        Ok(match self.byte()? {
            0 => Value::Bool(false),
            1 => Value::Bool(true),
            2 => Value::Number(u64::from_le_bytes(
                self.take(8)?
                    .try_into()
                    .map_err(|_| Error::new("TRUNCATED_BINARY"))?,
            )),
            3 => Value::Text(self.string()?),
            4 => Value::Symbol(self.string()?),
            5 => Value::List(self.values(depth)?),
            6 => Value::Call(Call {
                name: self.string()?,
                args: self.values(depth)?,
    ...
    fn calls(&mut self) -> Result<Vec<Call>> {
        let n = self.count(MAX_NODES)?;
        let mut out = Vec::new();
        for _ in 0..n {
            match self.value(0)? {
                Value::Call(c) => out.push(c),
                _ => return Err(Error::new("EXPECTED_CALL")),
            }
        }
        Ok(out)
    }
...
pub fn decode(bytes: &[u8]) -> Result<Document> {
    ...
    for _ in 0..count {
        let id = r.number()? as u32;
        let flags = r.byte()?;
        if flags > 1 {
            return Err(Error::new("INVALID_SECTION_FLAGS"));
        }
        if !ids.insert(id) {
            return Err(Error::new("DUPLICATE_SECTION"));
        }
        let b = r.bytes()?;
        if id == 1 {
            if flags != 1 {
            ...
            return Err(Error::new("UNSUPPORTED_CRITICAL_EXTENSION"));
        } else {
            extensions.push(tessembly_codec::Extension {
                id,
                bytes: b.to_vec(),
            });
        }
    ...
    doc.optional_extensions = extensions;
    doc.validate()?;
    Ok(doc)
}
