    ...
    fn write(n: &Node) -> String {
        let mut s = match &n.kind {
            ...
            NodeKind::Pool { mask, take } => {
                let kinds: String = Piece::ALL.into_iter().filter(|p| mask & p.bit() != 0).map(|p| p.ascii() as char).collect();
                format!("[{kinds}]{take}")
            }
        ...
        if n.constraints.draw.is_some() || n.constraints.use_order.is_some() {
            if matches!(n.kind, NodeKind::Concat(_) | NodeKind::Union(_)) { s = format!("{{{s}}}"); }
            s.push(':');
            if let Some(ps) = &n.constraints.draw { s.push_str(&format!("D({})", predicates(ps))); }
            if let Some(ps) = &n.constraints.use_order { s.push_str(&format!("U({})", predicates(ps))); }
        }
        s
    }
    Ok(write(root))
